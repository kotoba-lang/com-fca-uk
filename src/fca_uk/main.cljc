(ns fca_uk.main
  "Kotodama WASM entrypoint for the fca_uk clean-room actor (L4) — Clojure port.
  ...
  facts (`fca_uk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fca_uk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "fcauk_fil"
    :fields [:filerId :scheme :period :status]   :required [:scheme :period]
    :coerce {}                       :refs {}}
   {:entity "Citizen"     :plural "citizens"     :id-prefix "fcauk_cit"
    :fields [:nationalId :jurisdiction]         :required [:jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Service"     :plural "services"     :id-prefix "fcauk_ser"
    :fields [:name :department]                  :required [:name :department]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "fcauk_app"
    :fields [:citizenId :serviceId :status]      :required [:status]
    :coerce {}                       :refs {:citizenId "Citizen" :serviceId "Service"}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "fcauk_dat"
    :fields [:name :publisher :updatedAt]        :required [:name :publisher]
    :coerce {}                       :refs {}}
   {:entity "Identity"    :plural "identities"   :id-prefix "fcauk_ide"
...
(defn emit-facts
  "EAVT facts for one record: {\"fca_uk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fca_uk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fca_uk-compat/ready)
